...
import model.AbstractLexer.UnexpectedTokenException;
...
public class LexerTests {
	...
	public static Collection<Object[]> data() {
		return Arrays.asList(new Object[][] {
			...
			{ "Vvv <->-><--> 1", tokens(withLexeme(VARIABLE_NAME, "Vvv"), DOUBLE_ARROW, ARROW, UnexpectedTokenException.class) },
			{ "^vEVAL letEVAL ' V Pv & ^", tokens(CARET, withLexeme(VARIABLE_NAME, "vEVAL"), withLexeme(VARIABLE_NAME, "letEVAL"), APOSTROPHE, VEE, withLexeme(VARIABLE_NAME, "Pv"), UnexpectedTokenException.class) },
		});
	}
		...
		for (int i = 0; i < tokens.length; i++) {
			if (tokens[i] instanceof Token) {
				...
				assertEquals(
					...
					((Object[]) tokens[i])[1],
					String.valueOf(LEXER.LEXEME)
				);
			}
			else if (tokens[i] instanceof RuntimeException) {
				assertThrows(
					"Unexpected token " + (i+1) + " of " + tokens.length + " must throw runtime exception",
					(Class<RuntimeException>) tokens[i],
